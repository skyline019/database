...
bool MVCCSnapshot::is_visible(uint64_t created_lsn, uint64_t deleted_lsn, uint64_t creator_txn) const {
    ...
    if (creator_txn != 0) {
        const auto it = committed_txn_lsn.find(creator_txn);
        if (it != committed_txn_lsn.end() && it->second > snapshot_lsn) {
            return false;
        }
    }
...
MVCCSnapshot MVCCManager::create_snapshot(const bool read_committed) const {
    std::lock_guard<std::mutex> lg(mut_);
    MVCCSnapshot s(wal_.current_lsn(), std::unordered_set<uint64_t>(active_txns_));
    s.committed_txn_lsn = committed_txn_lsn_;
    if (read_committed) {
        // Statement-level view: caller should refresh per read.
        s.snapshot_lsn = wal_.current_lsn();
    }
