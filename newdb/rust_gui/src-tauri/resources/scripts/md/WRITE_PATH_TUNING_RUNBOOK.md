...
- `scripts/bench/concurrent_pressure_bench.ps1`
- `scripts/soak/test_loop.ps1` (optional high-pressure stage via `-RunHighPressure`)

Concurrent pressure benchmark now includes explicit profile/durability dimensions:

- `-BenchmarkProfile newdb-default|leveldb-like|innodb-like`
- `-RuntimeWalSyncMode off|normal|full`
- `-RuntimeWalSyncNormalIntervalMs <ms>` (used when mode is `normal`)

Summary output schema adds:

- `benchmark_profile`
- `runtime_walsync_mode`
- `runtime_walsync_normal_interval_ms`
- `runtime_pressure_tps_est`
- `runtime_pressure_batch_ms_p95`

This enables apples-to-apples gate checks across durability profiles.
...
Runtime compact quality gate example (recommended for soak/nightly):

```powershell
powershell -ExecutionPolicy Bypass -File ".\scripts\soak\test_loop.ps1" `
  -RunConcurrentPressure `
  -MaxRuntimeVacuumCompactFailureDelta 0 `
  -MinRuntimeVacuumCompactReclaimedBytesDelta -1 `
  -MaxRuntimeVacuumQueueDepthPeak 4 `
  -MaxRuntimeWalRecoveryLastElapsedMs 500
```

Nightly runner example (lite profile with compact failure hard gate):

```powershell
powershell -ExecutionPolicy Bypass -File ".\scripts\soak\nightly_soak_runner.ps1" `
  -LiteProfile `
  -MaxRuntimeVacuumCompactFailureDelta 0 `
  -MaxRuntimeVacuumQueueDepthPeak 4 `
  -MaxRuntimeWalRecoveryLastElapsedMs 500
...
- **LSM async compaction backpressure**: `NEWDB_LSM_COMPACTION_MAX_PENDING=<N>` caps queued compaction requests; when exceeded, enqueue is skipped and counters are recorded.
- **New runtime counters for steady-state diagnosis**:
