...
void TxnCoordinator::maybeCompactWalAfterCommit(const std::string& committed_table) {
    ...
    m_maintenance_checkpoint_trigger_count.fetch_add(1, std::memory_order_relaxed);
    if (!committed_table.empty() && m_vacuum_running.load(std::memory_order_relaxed)) {
        triggerVacuum(committed_table);
        m_maintenance_checkpoint_vacuum_enqueue_count.fetch_add(1, std::memory_order_relaxed);
    }
...
void TxnCoordinator::setGroupCommitWindowMs(const std::uint64_t ms) {
    ...
    m_group_commit_window_ms.store(ms, std::memory_order_relaxed);
}


std::uint64_t TxnCoordinator::groupCommitWindowMs() const {
    return m_group_commit_window_ms.load(std::memory_order_relaxed);
}
...
void TxnCoordinator::setGroupCommitMaxBatchCommits(const std::uint64_t n) {
    ...
    m_group_commit_max_batch_commits.store((n == 0) ? 1 : n, std::memory_order_relaxed);
}


std::uint64_t TxnCoordinator::groupCommitMaxBatchCommits() const {
    return m_group_commit_max_batch_commits.load(std::memory_order_relaxed);
}


void TxnCoordinator::setWalAdaptiveEnabled(const bool enabled) {
    m_wal_adaptive_enabled.store(enabled, std::memory_order_relaxed);
}


bool TxnCoordinator::walAdaptiveEnabled() const {
    return m_wal_adaptive_enabled.load(std::memory_order_relaxed);
}

void TxnCoordinator::setHybridAdaptiveEnabled(const bool enabled) {
    m_hybrid_adaptive_enabled.store(enabled, std::memory_order_relaxed);
}

bool TxnCoordinator::hybridAdaptiveEnabled() const {
    return m_hybrid_adaptive_enabled.load(std::memory_order_relaxed);
}


void TxnCoordinator::setHotIndexEnabled(const bool enabled) {
    m_hot_index_enabled.store(enabled, std::memory_order_relaxed);
}


bool TxnCoordinator::hotIndexEnabled() const {
    return m_hot_index_enabled.load(std::memory_order_relaxed);
}


void TxnCoordinator::setSegmentTargetBytes(const std::uint64_t bytes) {
    m_segment_target_bytes.store(bytes, std::memory_order_relaxed);
}


std::uint64_t TxnCoordinator::segmentTargetBytes() const {
    return m_segment_target_bytes.load(std::memory_order_relaxed);
}


void TxnCoordinator::setLsmSegmentCount(const std::uint64_t n) {
    m_lsm_segment_count.store(n, std::memory_order_relaxed);
}
