...
bool handle_txn_commands(ShellState& st, const char* line, const char* log_file, const std::string& current_table) {
    ...
    auto txn_backup_path = [&]() { return f.effective_data_path() + ".txn.bak"; };
    // BEGIN [table_name] - ????
    ...
    if (strcasecmp_ascii(line, "SHOW TUNING") == 0 || strcasecmp_ascii(line, "SHOW STATUS") == 0) {
        const TxnRuntimeStats stats = f.txn().runtimeStats();
        const auto mode = f.txn().walSyncMode();
        ...
        log_and_print(log_file,
                      ...
                      mode_s,
                      static_cast<unsigned long long>(f.txn().walNormalSyncIntervalMs()),
                      f.txn().vacuumRunning() ? "on" : "off",
                      f.txn().vacuumOpsThreshold(),
                      f.txn().vacuumMinIntervalSec(),
    ...
    if (strcasecmp_ascii(line, "SHOW TUNING JSON") == 0 || strcasecmp_ascii(line, "SHOW STATUS JSON") == 0) {
        const std::string json = newdb::capi_cli::format_runtime_stats_json(f);
    ...
    if (strncasecmp_ascii(line, "AUTOVACUUM", 10) == 0) {
        if (std::strlen(line) > 10) {
            ...
            if (mode == "1" || mode == "on") {
                ...
                try {
                    f.txn().setVacuumMinIntervalSec(static_cast<std::size_t>(std::stoull(threshold_token)));
    ...
    if (strncasecmp_ascii(line, "WALSYNC", 7) == 0) {
        ...
        if (lower == "off") {
            f.txn().setWalSyncMode(newdb::WalSyncMode::Off);
        } else if (lower == "normal") {
            f.txn().setWalSyncMode(newdb::WalSyncMode::Normal);
            if (!interval_token.empty()) {
                try {
                    f.txn().setWalNormalSyncIntervalMs(static_cast<std::uint64_t>(std::stoull(interval_token)));
                } catch (...) {
    ...
    if (strncasecmp_ascii(line, "TXNISOLATION", 12) == 0) {
        ...
        if (arg.empty()) {
            const auto level = f.txn().txnIsolationLevel();
        ...
        if (arg == "read_committed" || arg == "rc") {
            f.txn().setTxnIsolationLevel(TxnIsolationLevel::ReadCommitted);
            log_and_print(log_file, "[TXN] isolation set to read_committed\n");
            return true;
        }
        if (arg == "snapshot" || arg == "si") {
            f.txn().setTxnIsolationLevel(TxnIsolationLevel::Snapshot);
    ...
    if (strncasecmp_ascii(line, "WRITECONFLICT", 13) == 0) {
        ...
        if (mode.empty()) {
            const auto p = f.txn().writeConflictPolicy();
            log_and_print(log_file, "[TXN] write_conflict=%s timeout_ms=%llu\n",
                          p == WriteConflictPolicy::Wait ? "wait" : "reject",
                          static_cast<unsigned long long>(f.txn().writeConflictWaitTimeoutMs()));
            return true;
        }
        if (mode == "reject") {
            f.txn().setWriteConflictPolicy(WriteConflictPolicy::Reject);
            log_and_print(log_file, "[TXN] write_conflict policy set to reject\n");
            return true;
        }
        if (mode == "wait") {
            f.txn().setWriteConflictPolicy(WriteConflictPolicy::Wait);
            if (!timeout_token.empty()) {
                try {
                    f.txn().setWriteConflictWaitTimeoutMs(static_cast<std::uint64_t>(std::stoull(timeout_token)));
                } catch (...) {
    ...
    if (strncasecmp_ascii(line, "SEGMENT", 7) == 0) {
        ...
        if (arg.empty()) {
            log_and_print(log_file, "[LSM] segment_target_bytes=%llu\n",
                          static_cast<unsigned long long>(f.txn().segmentTargetBytes()));
            return true;
        }
        try {
            f.txn().setSegmentTargetBytes(static_cast<std::uint64_t>(std::stoull(arg)));
    ...
    if (strncasecmp_ascii(line, "WALADAPTIVE", 11) == 0) {
        ...
        f.txn().setWalAdaptiveEnabled(*on);
    ...
    if (strncasecmp_ascii(line, "GROUPCOMMIT", 11) == 0) {
        ...
        if (window_ms_token.empty()) {
            log_and_print(log_file, "[WAL] group_commit window_ms=%llu max_batch=%llu\n",
                          static_cast<unsigned long long>(f.txn().groupCommitWindowMs()),
                          static_cast<unsigned long long>(f.txn().groupCommitMaxBatchCommits()));
            return true;
        }
        try {
            f.txn().setGroupCommitWindowMs(static_cast<std::uint64_t>(std::stoull(window_ms_token)));
            if (!max_batch_token.empty()) {
                f.txn().setGroupCommitMaxBatchCommits(static_cast<std::uint64_t>(std::stoull(max_batch_token)));
            }
