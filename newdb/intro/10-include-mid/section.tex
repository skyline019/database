\section{中层解析 II：engine/include（公共 API 与契约层）}

\subsection{模块定位}

\fpath{newdb/engine/include/newdb/*.h} 是工程的“公共契约层”：

\begin{itemize}
  \item 向 \fpath{cli/}、\fpath{tests/}、\fpath{tools/} 暴露稳定接口；
  \item 隐藏 \fpath{newdb/engine/src/} 的实现细节（例如文件布局、线程模型、平台差异）；
...
\begin{enumerate}
  ...
  \item \textbf{读路径预算（可选）}：\fpath{page_cache.h}、\fpath{memory_budget.h}（惰性堆解码路径上的页缓存与总字节预算）
  \item \textbf{编解码与 sidecar 层}：\fpath{tuple_codec.h}、\fpath{wal_codec.h}、\fpath{schema_io.h}；WAL 段扫描声明 \fpath{wal/wal_segment_scanner.h}
