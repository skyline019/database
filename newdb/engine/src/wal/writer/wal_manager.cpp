...
bool crash_injected(const char* point) {
    const char* raw = std::getenv("NEWDB_WAL_CRASH_POINT");
    return raw != nullptr && std::strcmp(raw, point) == 0;
}
...
Status WalManager::append_record(uint64_t txn_id, WalOp op, const std::string& table,
                                 ...
                                 const std::uint64_t db_object_id,
                                 const std::uint64_t savepoint_id,
                                 const std::uint64_t undo_prev_lsn,
                                 const std::uint64_t pitr_target_lsn,
                                 const std::uint64_t pitr_target_ts_ms,
                                 const std::uint64_t op_seq_in_txn) {
    return append_record_with_lsn(txn_id, op, table, row, raw_payload, before_row, after_row,
                                  db_object_id, savepoint_id, undo_prev_lsn, pitr_target_lsn, pitr_target_ts_ms,
                                  op_seq_in_txn, nullptr);
}

Status WalManager::append_record_with_lsn(uint64_t txn_id, WalOp op,
    ...
    uint32_t before_row_id_u32 = 0;
    uint32_t after_row_id_u32 = 0;
    const uint32_t* row_id = nullptr;
    walcodec::RowImageView before_view{};
    walcodec::RowImageView after_view{};
    walcodec::PayloadMetaView meta_view{};
    const walcodec::RowImageView* before_ptr = nullptr;
    const walcodec::RowImageView* after_ptr = nullptr;
    const walcodec::PayloadMetaView* meta_ptr = nullptr;
    const std::uint64_t record_ts_ms = WalManager::wall_clock_ms();
    if (row != nullptr) {
    ...
    if (before_row != nullptr) {
        before_row_id_u32 = static_cast<uint32_t>(before_row->id);
        Status st = encode_row_payload(*before_row, TableSchema{}, before_bytes);
        if (!st.ok) return st;
        before_view.row_id = &before_row_id_u32;
        before_view.row_payload = &before_bytes;
        before_ptr = &before_view;
    }
    if (after_row != nullptr) {
        after_row_id_u32 = static_cast<uint32_t>(after_row->id);
        Status st = encode_row_payload(*after_row, TableSchema{}, after_bytes);
        if (!st.ok) return st;
        after_view.row_id = &after_row_id_u32;
        after_view.row_payload = &after_bytes;
        after_ptr = &after_view;
    }
    if (before_ptr == nullptr && after_ptr == nullptr && row_id != nullptr && row_payload != nullptr) {
        // Compat path: old caller only provides one row image.
        after_view.row_id = row_id;
        after_view.row_payload = row_payload;
        after_ptr = &after_view;
    }
    if (db_object_id != 0 || savepoint_id != 0 || undo_prev_lsn != 0 ||
        pitr_target_lsn != 0 || pitr_target_ts_ms != 0 || record_ts_ms != 0) {
        if (db_object_id != 0) meta_view.db_object_id = &db_object_id;
        if (savepoint_id != 0) meta_view.savepoint_id = &savepoint_id;
        if (undo_prev_lsn != 0) meta_view.undo_prev_lsn = &undo_prev_lsn;
        if (pitr_target_lsn != 0) meta_view.pitr_target_lsn = &pitr_target_lsn;
        if (pitr_target_ts_ms != 0) meta_view.pitr_target_ts_ms = &pitr_target_ts_ms;
        if (record_ts_ms != 0) meta_view.record_ts_ms = &record_ts_ms;
        meta_ptr = &meta_view;
    }

    std::vector<uint8_t> payload;
    Status payload_st = walcodec::build_payload(table, before_ptr, after_ptr, meta_ptr, op_seq_in_txn, payload);
...
Status WalManager::flush_durable_nolock() {
    ...
    if (crash_injected("flush_before")) return Status::Fail("injected crash: flush_before");
    if (std::fflush(fp_) != 0) return Status::Fail("fflush failed");
    if (crash_injected("flush_after")) return Status::Fail("injected crash: flush_after");
...
Status WalManager::checkpoint(uint64_t snapshot_lsn) {
    const Status begin_st = checkpoint_begin(snapshot_lsn);
    if (!begin_st.ok) return begin_st;
    return checkpoint_end(snapshot_lsn);
}
...
Status WalManager::checkpoint_end(uint64_t snapshot_lsn) {
    ...
    return append_record(0, WalOp::PITR_MARK, /*table=*/"", nullptr, nullptr, nullptr, nullptr,
                         /*db_object_id=*/0, /*savepoint_id=*/0, /*undo_prev_lsn=*/0,
                         /*pitr_target_lsn=*/snapshot_lsn,
                         /*pitr_target_ts_ms=*/WalManager::wall_clock_ms(),
                         /*op_seq_in_txn=*/0);
}
...
Status WalManager::checkpoint_and_truncate(uint64_t snapshot_lsn) {
    const Status cp_begin = checkpoint_begin(snapshot_lsn);
    if (!cp_begin.ok) return cp_begin;
    if (crash_injected("checkpoint_between_begin_end")) {
        return Status::Fail("injected crash: checkpoint_between_begin_end");
    }
    const Status fst = flush();
    if (!fst.ok) return fst;
    const Status cp_end = checkpoint_end(snapshot_lsn);
    if (!cp_end.ok) return cp_end;
    const Status fst2 = flush();
        ...
        while (true) {
            ...
            if (!payload.empty()) {
                ...
                if (walcodec::is_v1_payload(payload.data(), payload.size())) {
                    walcodec::DecodedPayloadV1 decv1{};
                    Status dec = walcodec::decode_payload_v1(payload.data(), payload.size(), decv1);
                    if (!dec.ok) return Status::Fail("decode v1 payload failed: " + dec.message);
                    rec.table = std::move(decv1.table);
                    rec.op_seq_in_txn = decv1.op_seq_in_txn;
                    rec.has_db_object_id = decv1.has_db_object_id;
                    rec.db_object_id = decv1.db_object_id;
                    rec.has_savepoint_id = decv1.has_savepoint_id;
                    rec.savepoint_id = decv1.savepoint_id;
                    rec.has_undo_prev_lsn = decv1.has_undo_prev_lsn;
                    rec.undo_prev_lsn = decv1.undo_prev_lsn;
                    rec.has_pitr_target_lsn = decv1.has_pitr_target_lsn;
                    rec.pitr_target_lsn = decv1.pitr_target_lsn;
                    rec.has_pitr_target_ts_ms = decv1.has_pitr_target_ts_ms;
                    rec.pitr_target_ts_ms = decv1.pitr_target_ts_ms;
                    rec.has_record_ts_ms = decv1.has_record_ts_ms;
                    rec.record_ts_ms = decv1.record_ts_ms;
                    if (decv1.has_before) {
                        Row row;
                        Status dr = decode_row_payload(decv1.before_row_payload, decv1.before_row_payload_len, row, schema);
                        if (!dr.ok) return dr;
                        row.id = static_cast<int>(decv1.before_row_id);
                        rec.before_row = row;
                        rec.has_before_row = true;
                        rec.row = row;
                        rec.has_row = true;
                    }
                    if (decv1.has_after) {
                        Row row;
                        Status dr = decode_row_payload(decv1.after_row_payload, decv1.after_row_payload_len, row, schema);
                        if (!dr.ok) return dr;
                        row.id = static_cast<int>(decv1.after_row_id);
                        rec.after_row = row;
                        rec.has_after_row = true;
                        rec.row = row;
                        rec.has_row = true;
                    }
                } else {
...
Status WalManager::write_record(const WalRecordHeader* hdr, const uint8_t* payload, uint32_t paylen) {
    if (crash_injected("commit_before_write")) return Status::Fail("injected crash: commit_before_write");
    ...
    if (crash_injected("commit_after_write")) return Status::Fail("injected crash: commit_after_write");
    return Status::Ok();
}
...
Status WalManager::append_control_record_nolock(uint64_t txn_id, WalOp op) {
    ...
    if (!writer_started_.load(std::memory_order_acquire)) {
        ...
        hdr.type = static_cast<uint8_t>(op);
        hdr.checksum = compute_checksum(&hdr, payload.data(), static_cast<std::uint32_t>(payload.size()));
        const Status wr = write_record(&hdr, payload.data(), static_cast<std::uint32_t>(payload.size()));
        if (!wr.ok) return wr;
...
Status WalManager::maybe_rotate_segment_nolock() {
    ...
    if (crash_injected("rotate_before")) return Status::Fail("injected crash: rotate_before");
    ...
    if (crash_injected("rotate_after")) return Status::Fail("injected crash: rotate_after");
    return Status::Ok();
}
