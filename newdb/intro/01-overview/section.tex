...
图~\ref{fig:newdb-end-to-end-dataflow} 概括\textbf{整个项目}从交互入口到引擎存储的主数据流（细部分支可在后续章节展开）。依赖方向与仓库约定一致：\fpath{cli/app} $\rightarrow$ \fpath{cli/shell} $\rightarrow$ \fpath{cli/modules} $\rightarrow$ \fpath{engine/include/newdb}（见 \fpath{docs/architecture/MODULE_BOUNDARIES.md}、\fpath{docs/architecture/NEWDB_DESIGN_POINT_TO_FILE_MAP.md}）。\textbf{跨模块数据流总览}（含 \fpath{waterfall/}、\fpath{rust_gui/}、脚本与 CI 观测链）见 \fpath{docs/architecture/PROJECT_DATAFLOW_WHOLE.md}。

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
  font=\small,
  nd/.style={rectangle, draw=black!55, rounded corners=3pt,
    inner xsep=8pt, inner ysep=6pt, align=left, text width=0.92\linewidth},
  arr/.style={-{Stealth[length=2mm]}, semithick, black!45},
]
\node[nd] (n1) {\textbf{入口}\quad \texttt{demo\_main}（CLI 参数与模式）、交互式 REPL（\fpath{cli/shell/repl/demo_shell.cc}）、以及引擎 \texttt{C API}（\fpath{engine/src/api/c/c_api.cpp}），最终进入同一条命令分发管线。};
\node[nd, below=7mm of n1] (n2) {\textbf{ShellState（会话上下文）}\quad 聚合 \texttt{Session}、\texttt{TxnCoordinator}、\texttt{WhereQueryContext}，以及 LSM-lite / sidecar 等会话级状态（\fpath{cli/shell/state/shell_state.h}）。};
\node[nd, below=7mm of n2] (n3) {\textbf{路由：\texttt{process\_command\_line}}\quad Phase\,1：无需已装载 \texttt{HeapTable} 的命令先尝试（\fpath{cli/shell/dispatch/router/dispatch.cc}）；Phase\,2：需表时经 \texttt{get\_cached\_table}/\texttt{Session::lock\_heap}。可与 \fpath{dispatch_routing.cc} 的 phase-2 前缀判定组合，跳过整条 Phase\,1 handler 链。};
\node[nd, below=7mm of n3] (n4) {\textbf{handlers（业务编排）}\quad DDL、事务与 workspace、DML、WHERE/PAGE/FIND 等查询、导入导出（\fpath{cli/shell/dispatch/handlers/}）。};
\node[nd, below=7mm of n4] (n5) {\textbf{cli modules}\quad 事务侧 WAL、文件锁、vacuum、recovery（\fpath{cli/modules/txn/coordinator/}）；\texttt{where} 解析与执行；sidecar（覆盖索引、可见性检查点等）；\texttt{lsm\_lite\_service} 等。};
\node[nd, below=7mm of n5] (n6) {\textbf{引擎}\quad \texttt{Session::ensure\_loaded}/\texttt{reload}；\texttt{HeapTable} 与页级 I/O（\fpath{engine/src/heap/}、\fpath{engine/src/io/page/}）；可选 \texttt{page\_cache}/\texttt{memory\_budget}（\fpath{engine/src/cache/}）；\texttt{WalManager} 编解码与恢复辅助（\fpath{engine/src/wal/recovery/}）；MVCC 快照与读路径可见性过滤。};
...
\end{tikzpicture}
\caption{newdb 端到端处理流程（主路径概览）}
\label{fig:newdb-end-to-end-dataflow}
\end{figure}

\textbf{读路径}多在 Phase\,2 $\rightarrow$ \texttt{where} + sidecar（含可选 \texttt{table\_stats} 提示与 \texttt{SHOW PLAN} JSON）$\rightarrow$ 惰性解码或索引访问 \texttt{HeapTable}。\textbf{写路径}经 \texttt{TxnCoordinator} $\rightarrow$ WAL 追加 $\rightarrow$ 堆文件持久化，并触发 sidecar/缓存失效策略；\texttt{SHOW TUNING JSON} 等路径导出运行时计数（含 \texttt{table\_storage\_health\_*}）。
...
定义见 \fpath{newdb/engine/include/newdb/row.h}：
...
定义见 \fpath{newdb/engine/include/newdb/heap_storage.h}：
...
定义见 \fpath{newdb/engine/include/newdb/mvcc.h}：
...
定义见 \fpath{newdb/engine/include/newdb/schema.h}：
...
定义见 \fpath{newdb/engine/include/newdb/heap_table.h}。最关键的是它同时承载两种表示：
...
\texttt{WalRecordHeader} 定义见 \fpath{newdb/engine/include/newdb/wal_manager.h}：
...
定义见 \fpath{newdb/engine/include/newdb/wal_manager.h}：
...
定义见 \fpath{newdb/engine/include/newdb/heap_file_read_view.h}：
...
定义见 \fpath{newdb/engine/include/newdb/heap_page.h}：
...
定义见 \fpath{newdb/engine/include/newdb/error.h}：
...
定义见 \fpath{newdb/engine/include/newdb/session.h}。它的“关键点”不是字段数量，而是并发语义：
...
定义见 \fpath{newdb/engine/include/newdb/catalog.h}：
...
\subsection{重构后新增能力（架构视角）}

相较早期“单路径回放”形态，当前工程在事务与恢复面上已补齐若干关键能力：

\begin{itemize}
  \item \textbf{恢复闭环}：WAL 从单一回放扩展为 analysis/redo/undo 三阶段恢复；实现上辅以段扫描与恢复支持单元（\fpath{wal\_segment\_scanner.*}、\fpath{wal\_manager\_recover\_support.cpp}），并与 \texttt{WalRecoveryStats} 可观测字段对齐。
  \item \textbf{任意回退}：命令层支持 savepoint 粒度回退/重做与事务级回滚；
  \item \textbf{时间点恢复}：支持按 LSN/时间戳约束回放终点（PITR）；
  \item \textbf{读路径预算}：引擎侧可选页缓存与内存预算（环境变量开关），降低重复解码与 sidecar 加载成本。
  \item \textbf{查询与存储可观测性}：\texttt{where} 路径集成表级统计（\texttt{*.tablestats}、\texttt{SHOW PLAN} JSON）；\texttt{table\_storage\_health} 与 \texttt{SHOW TUNING JSON} 中的存储健康字段形成运维闭环。
  \item \textbf{CLI 分发结构}：\fpath{cli/shell/dispatch/} 拆分为 \texttt{router/}（入口与 phase-2 前缀快路径）、\texttt{registry/}（命令表与快照）、\texttt{handlers/}、\texttt{support/}、\texttt{services/}、\texttt{shared/}，降低单文件膨胀风险。
  \item \textbf{ShellState / WHERE plan 源码解耦（非 GUI）}：\texttt{ShellState} 侧采用 Facade、Ops、pimpl 与 Owner 分层（\fpath{cli/shell/state/}），配合 include 收敛门禁（\fpath{docs/dev/SHELL_STATE_INCLUDE_AUDIT.md}）；\texttt{where/executor/plan/} 将查询计划拆为多编译单元（\texttt{plan\_impl\_support}、\texttt{plan\_query\_index}、\texttt{plan\_scan\_estimate}、\texttt{where\_plan\_catalog} 等），降低单 TU 体量。
  \item \textbf{\fpath{waterfall/} 与 CRC32C}：页式存储与通用工具库；CRC32C 兼容实现集中于 \texttt{crc32c\_compat}（系统无 \texttt{libcrc32c} 时编入）。CMake 对静态库 \texttt{waterfall} 启用 \texttt{POSITION\_INDEPENDENT\_CODE}，以便链入 \texttt{libnewdb.so} / \texttt{libnewdb\_cli\_backend.so}。总览见 \fpath{docs/architecture/PROJECT_DATAFLOW_WHOLE.md}（§2.0、§2.2.2）。
  \item \textbf{GUI 运维化}：Rust GUI 提供事务栈、回看、崩溃矩阵与趋势看板入口，形成“执行-验证-复盘”链路；顶栏菜单按功能分组，并暴露 \texttt{SHOW TUNING JSON}、\texttt{SHOW STORAGE}、\texttt{SHOW PLAN}/\texttt{EXPLAIN WHERE} 等与会话调优相关的命令入口。
  \item \textbf{文档索引}：\fpath{newdb/docs/} 按主题分子目录（\texttt{api/}、\texttt{architecture/}、\texttt{dev/}、\texttt{ci/}、\texttt{txn/} 等），便于与 intro 交叉阅读。
\end{itemize}
