...
图~\ref{fig:newdb-end-to-end-dataflow} 概括\textbf{整个项目}从交互入口到引擎存储的主数据流（细部分支可在后续章节展开）。依赖方向与仓库约定一致：\fpath{cli/app} $\rightarrow$ \fpath{cli/shell} $\rightarrow$ \fpath{cli/modules} $\rightarrow$ \fpath{engine/include/newdb}（见 \fpath{docs/architecture/MODULE_BOUNDARIES.md}、\fpath{docs/architecture/NEWDB_DESIGN_POINT_TO_FILE_MAP.md}）。
...
\node[nd, below=7mm of n5] (n6) {\textbf{引擎}\quad \texttt{Session::ensure\_loaded}/\texttt{reload}；\texttt{HeapTable} 与页级 I/O（\fpath{engine/src/heap/}、\fpath{engine/src/io/page/}）；\texttt{WalManager} 编解码；MVCC 快照与读路径可见性过滤。};
...
\textbf{读路径}多在 Phase\,2 $\rightarrow$ \texttt{where} + sidecar $\rightarrow$ 惰性解码或索引访问 \texttt{HeapTable}。\textbf{写路径}经 \texttt{TxnCoordinator} $\rightarrow$ WAL 追加 $\rightarrow$ 堆文件持久化，并触发 sidecar/缓存失效策略。
...
\begin{itemize}
  \item \textbf{恢复闭环}：WAL 从单一回放扩展为 analysis/redo/undo 三阶段恢复；
  \item \textbf{任意回退}：命令层支持 savepoint 粒度回退/重做与事务级回滚；
  \item \textbf{时间点恢复}：支持按 LSN/时间戳约束回放终点（PITR）；
  \item \textbf{GUI 运维化}：Rust GUI 提供事务栈、回看、崩溃矩阵与趋势看板入口，形成“执行-验证-复盘”链路。
\end{itemize}
