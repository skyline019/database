...
ShellState::BenchmarkProfile parse_profile(const std::string& raw) {
    if (raw == "leveldb-like") return ShellState::BenchmarkProfile::LeveldbLike;
    if (raw == "innodb-like") return ShellState::BenchmarkProfile::InnodbLike;
    if (raw == "hybrid-balanced") return ShellState::BenchmarkProfile::HybridBalanced;
    return ShellState::BenchmarkProfile::NewdbDefault;
}

ShellState::BenchmarkProfile resolve_profile() {
    const char* raw = std::getenv("NEWDB_BENCHMARK_PROFILE");
    if (!raw || !*raw) return ShellState::BenchmarkProfile::NewdbDefault;
    std::string s(raw);
    std::transform(s.begin(), s.end(), s.begin(), [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
    return parse_profile(s);
}

void apply_profile_defaults_once(ShellState& st, const ShellState::BenchmarkProfile profile) {
    if (st.runtime_policy.initialized && st.runtime_policy.profile == profile) {
        return;
    }
    st.runtime_policy.profile = profile;
    st.runtime_policy.initialized = true;
    switch (profile) {
    case ShellState::BenchmarkProfile::LeveldbLike:
        st.txn.setWalSyncMode(newdb::WalSyncMode::Normal);
        st.txn.setWalNormalSyncIntervalMs(40);
        st.txn.setGroupCommitWindowMs(30);
        st.txn.setGroupCommitMaxBatchCommits(64);
        st.txn.setWalAdaptiveEnabled(false);
        st.txn.setHybridAdaptiveEnabled(false);
        break;
    case ShellState::BenchmarkProfile::InnodbLike:
        st.txn.setWalSyncMode(newdb::WalSyncMode::Full);
        st.txn.setWalNormalSyncIntervalMs(0);
        st.txn.setGroupCommitWindowMs(0);
        st.txn.setGroupCommitMaxBatchCommits(1);
        st.txn.setWalAdaptiveEnabled(false);
        st.txn.setHybridAdaptiveEnabled(false);
        break;
    case ShellState::BenchmarkProfile::HybridBalanced:
        st.txn.setWalSyncMode(newdb::WalSyncMode::Normal);
        st.txn.setWalNormalSyncIntervalMs(20);
        st.txn.setGroupCommitWindowMs(10);
        st.txn.setGroupCommitMaxBatchCommits(16);
        st.txn.setWalAdaptiveEnabled(true);
        st.txn.setHybridAdaptiveEnabled(true);
        break;
    case ShellState::BenchmarkProfile::NewdbDefault:
        st.txn.setWalSyncMode(newdb::WalSyncMode::Normal);
        st.txn.setWalNormalSyncIntervalMs(20);
        st.txn.setGroupCommitWindowMs(10);
        st.txn.setGroupCommitMaxBatchCommits(16);
...
newdb::lsm_lite::CompactionPolicy policy_for_profile(const ShellState::BenchmarkProfile p) {
    if (p == ShellState::BenchmarkProfile::LeveldbLike) {
        return newdb::lsm_lite::CompactionPolicy::SizeTiered;
    }
    return newdb::lsm_lite::CompactionPolicy::LeveledLite;
}

std::string profile_name(const ShellState::BenchmarkProfile p) {
    switch (p) {
    case ShellState::BenchmarkProfile::LeveldbLike: return "leveldb-like";
    case ShellState::BenchmarkProfile::InnodbLike: return "innodb-like";
    case ShellState::BenchmarkProfile::HybridBalanced: return "hybrid-balanced";
    case ShellState::BenchmarkProfile::NewdbDefault:
    default: return "newdb-default";
    }
}

newdb::lsm_lite::Options build_options(const ShellState& st) {
    newdb::lsm_lite::Options opt;
    opt.enabled = st.txn.hotIndexEnabled();
    opt.segment_target_bytes = st.txn.segmentTargetBytes();
    ...
    const auto p = st.runtime_policy.profile;
...
newdb::lsm_lite::Hooks build_hooks(ShellState& st) {
    ...
    h.on_memtable_flush = [&]() { st.txn.onLsmMemtableFlush(); };
    h.on_compaction = [&]() { st.txn.onLsmCompaction(); };
    h.on_read_segments_scanned = [&](const std::uint64_t n) { st.txn.onLsmReadSegmentsScanned(n); };
    h.on_cache_lookup = [&](const bool hit) { st.txn.onLsmSegmentCacheLookup(hit); };
    h.on_compaction_queue_depth = [&](const std::uint64_t p, const std::uint64_t i) {
        st.txn.onLsmCompactionQueueDepth(p, i);
    };
    h.on_compaction_enqueue_skipped_backpressure = [&]() { st.txn.onLsmCompactionEnqueueSkippedBackpressure(); };
    h.on_compaction_bytes = [&](const std::uint64_t inb, const std::uint64_t outb) {
        st.txn.onLsmCompactionBytes(inb, outb);
    };
    h.on_memtable_bytes = [&](const std::uint64_t n) { st.txn.setLsmMemtableBytes(n); };
    h.on_segment_count = [&](const std::uint64_t n) { st.txn.setLsmSegmentCount(n); };
    return h;
}

newdb::lsm_lite::TxnContext build_txn_ctx(const ShellState& st) {
    newdb::lsm_lite::TxnContext t;
    t.in_txn = st.txn.inTransaction();
    t.txn_id = st.txn.getTxnId();
    return t;
}
} // namespace
