...
1. Launch `structdb-rust-vue-gui.exe` (or dev `cargo tauri dev` from `gui/rust_gui/`).
2. Set workspace to a clean folder, e.g. `C:/tmp/structdb_gui_case`.
...
```text
IMPORTDIR(C:/tmp/structdb_gui_case)
SHOW TABLES
```
...
Executed in ASCII workspace `C:/tmp/structdb_gui_build` with looped run/fix/retest.
...
## 14) MDB script progress / cancel + SCAN cursor (2026-05)

**Script (MDB editor tab)**

1. Paste a multi-line script with several non-empty commands.
2. Click **执行脚本**; observe progress text `lineDone/totalLines` (from event `mdb-script-progress`).
3. Click **停止脚本** during execution; confirm dialog mentions cancellation and output contains stop reason.

**SCAN cursor (command box, after `USE` a table with ≥5 rows)**
...
Expected: first two invocations each log `rows_shown=2` and advancing `cursor=`; `SCAN RESET` logs cursor reset.

Regression: `structdb_tests --gtest_filter=Mdb.ScanMoreCursorPaging` (or full suite).
