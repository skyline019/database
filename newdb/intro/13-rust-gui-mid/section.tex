...
\begin{itemize}
  \item \texttt{sync\_runtime\_binaries.ps1} 统一同步 \texttt{newdb\_demo}/\texttt{newdb\_perf}/\texttt{newdb\_runtime\_report}/\texttt{libnewdb.dll}/\texttt{libgtest\_capi.dll} 与脚本、results 种子文件；构建产物支持在构建根、\texttt{bin/}、以及 MSVC 多配置子目录（\texttt{Release/} 等）中自动解析。
...
\begin{itemize}
  \item \textbf{栈模型升级}：以 savepoint 级 \texttt{UndoUnit} 聚合命令明细 \texttt{UndoOp}，支持跨表事务回退；
  \item \textbf{执行策略}：后端区分 soft/hard fail，失败时按策略修复或中止，避免半回退状态静默扩散；
  \item \textbf{持久化恢复}：事务栈以 jsonl 持久化，启动时可恢复历史并对坏行容错；
  \item \textbf{逆向推断增强}：针对 \texttt{UPDATE/DELETE/SETATTR} 等操作优先走结构化查询快照推断 inverse，降低文本解析误差；
  \item \textbf{可观测性}：回看面板支持命中导航、会话分段、逆向来源展示，便于审计与复盘。
\end{itemize}
