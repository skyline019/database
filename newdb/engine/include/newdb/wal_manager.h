...
struct WalRecoveryStats {
    ...
    std::uint64_t redo_records_applied{0};
    std::uint64_t redo_records_skipped{0};
    std::uint64_t undo_records_applied{0};
    std::uint64_t checkpoint_begin_count{0};
    std::uint64_t checkpoint_end_count{0};
...
public:
    ...
    Status append_record(uint64_t txn_id, WalOp op,
                         ...
                         const Row* before_row = nullptr,
                         const Row* after_row = nullptr,
                         std::uint64_t db_object_id = 0,
                         std::uint64_t savepoint_id = 0,
                         std::uint64_t undo_prev_lsn = 0,
                         std::uint64_t pitr_target_lsn = 0,
    ...
    // Same as append_record, but also returns the assigned record LSN.
    Status append_record_with_lsn(uint64_t txn_id, WalOp op,
                                  const std::string& table,
                                  const Row* row,
                                  const std::vector<uint8_t>* raw_payload,
                                  const Row* before_row,
                                  const Row* after_row,
                                  std::uint64_t db_object_id,
                                  std::uint64_t savepoint_id,
                                  std::uint64_t undo_prev_lsn,
                                  std::uint64_t pitr_target_lsn,
                                  std::uint64_t pitr_target_ts_ms,
    ...
    Status checkpoint_begin(uint64_t snapshot_lsn);
    Status checkpoint_end(uint64_t snapshot_lsn);
