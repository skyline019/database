...
\begin{itemize}
  ...
  \item \textbf{PHASE40 persist（四十期）}：\texttt{Mdb.Phase40*}（\textbf{24} 项）——分块元数据、10K 延迟索引、嵌套 bulk、多表 FLUSH、WAL 尾/损坏帧、增量 persist/coalesce/embed 分帧/checkpoint；更广 \texttt{Mdb.*} \textbf{127} 项全绿（2026-05-16）。
  \item \textbf{存储 import raw + 分帧}：\texttt{StorageEngine.WalReplayImportRaw*}、\texttt{WalReplay*ChunkedMdb*}、\texttt{CommitEmbedBatchAutoSplit*}。
\end{itemize}
...
structdb_tests --gtest_filter="StorageEngine.WalReplayImportRaw*:StorageEngine.WalReplay*Chunked*"
\end{lstlisting}
