...
\fpath{src/client/mdb/src/mdb_persistence.cpp} & \texttt{persist\_table}、\texttt{persist\_table\_chunked}、\texttt{build/submit\_persist\_command\_batch}、plain/raw 导入提示；\texttt{mdb\_storage\_read\_seq}。 \\
...
\begin{itemize}
  ...
  \item \texttt{submit(batch, fsync\_journal, error\_out)} / \texttt{submit\_ex(..., write\_journal)}：写引擎 + 可选 journal；大导入 plain 批次可 \texttt{write\_journal=false}（WAL 权威）。\texttt{fsync\_journal==true} 时在批次末尾 \texttt{wal\_sync()}（耐久 Level 1 类比，见 \fpath{Docs/phases/TXN_INNODB_MAP.md}）。
