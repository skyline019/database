...
std::vector<std::size_t> page_indices_keyset_after_id(const newdb::HeapTable& tbl,
                                                      ...
                                                      const int after_id,
                                                      const bool use_keyset) {
    if (!use_keyset) {
        return sorted_indices;
    }
    std::size_t start = 0;
    for (; start < sorted_indices.size(); ++start) {
        newdb::Row r;
        if (!row_at_slot(tbl, sorted_indices[start], r)) {
            continue;
        }
        if (descending) {
            if (r.id < after_id) {
    ...
    return std::vector<std::size_t>(sorted_indices.begin() + static_cast<std::ptrdiff_t>(start),
                                    sorted_indices.end());
}
