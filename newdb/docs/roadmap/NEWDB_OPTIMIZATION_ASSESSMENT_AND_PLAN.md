...
| 阶段 2 | 🟢 | 🟢 默认 checkpoint 剪枝（`NEWDB_RECOVER_USE_CHECKPOINT_LSN=0` 关闭）、**扩展 fault 用例**（含 **多 segment**、**不完整 checkpoint + 未提交 insert**、孤立 `CHECKPOINT_END`、双 `BEGIN` 无 `END`、`NEWDB_RECOVER_MIN_LSN`、**v2 闭环新增**：**`MultiSegmentMiddleCorruptedReplay` / `CrossSegmentUncommittedTxnDangling` / `CheckpointBeginNoEndKeepsCpDepth`**）；🟢 **WAL recover 类拆分**：[`WalRedoPlanner`](../../engine/include/newdb/wal/wal_redo_planner.h)（feed_record/finalize/`RedoPlanSummary`）+ [`WalRedoApplier`](../../engine/include/newdb/wal/wal_redo_applier.h)（`ApplyStats`），`recover_replay_segments` 行为不变；🟢 **Nightly + PR 可选**恢复耗时门（**`--max-wal-recovery-last-elapsed-ms`**）；runtime 已含 **`wal_recovery_redo_ms` / `wal_recovery_apply_count` / checkpoint 计数**；🟡 **全员 PR 默认**恢复硬门（仍按 [`PERF_AND_CI_BUDGETS.md`](../ci/PERF_AND_CI_BUDGETS.md) §4 软门策略） |
...
| 阶段 5 | 🟢 | 🟢 单页缓存与 **`heap_decode_slot_*`** 观测、**`MemoryRegistry`** 三类（[`MemoryKind::PageCache`](../../engine/include/newdb/memory_registry.h) / `EqSidecar` / `QueryTemp`）统一 `try_admit/release` + per-kind cap + 全局 cap + LRU evictor；**`page_cache_*`** + **`mem_*_{used_bytes,evictions,admit_rejects}`** + **`mem_global_*`** runtime 字段；**`where_eq_sidecar_disk_*`**（sidecar 磁盘读观测）；env：**`NEWDB_PAGE_CACHE_MAX_BYTES` / `NEWDB_SIDECAR_CACHE_MAX_BYTES` / `NEWDB_QUERY_TEMP_MAX_BYTES` / `NEWDB_MEMORY_BUDGET_MAX_BYTES`**；🟡 列级 histogram 驱动的 query temp 精细估算 |
| 阶段 6 | 🟢 | 🟢 既有 IndexCatalog / **`NEWDB_INDEX_CATALOG_ENFORCE=1`** 路径、**`BUILD.md` CI 文档化**、GitHub **`linux-index-catalog-enforce`** job、**eq sidecar / bloom 原子写**、头行 **`;bld=`（写入 `ready`）**、**`tests/test_index_catalog.cpp`** 对 **`bld=1/2`** 解析；🟢 **跨机基线**：[`ci_bench_gate.py`](../../scripts/ci/ci_bench_gate.py) 新参数 **`--baseline-host-index` / `--baseline-prefer-host-slug`**（最近邻 host_slug → 3 段 → 2 段 → 最新 fallback），与 [`capture_baseline.py`](../../scripts/ci/capture_baseline.py) 的 `--cross-host-baseline-dir` host_index 对接；reusable workflow 在 `linux-bench-gate-runtime-contract` / `linux-release-gate` 中下载 manifest artifact 并透传索引；`gate-fail-json-out` 含 **`baseline_host_slug_used`**；🟡 enforce 仍非默认全 PR、完整跨进程 **building→ready** 协调 |
