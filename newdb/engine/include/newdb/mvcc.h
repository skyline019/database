...
class WalManager;
class HeapTable;
enum class TxnIsolationLevel : std::uint8_t;
...
struct MVCCSnapshot {
    ...
    std::unordered_map<uint64_t, uint64_t> committed_txn_lsn; // txn -> commit_lsn watermark
...
public:
    ...
    // `read_committed=true` returns a "fresh" read view for each statement.
    MVCCSnapshot create_snapshot(bool read_committed = false) const;
