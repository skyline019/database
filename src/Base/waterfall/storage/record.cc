...
#if defined(_WIN32)
#  include <cstdint>
// Windows MSVC targets are little-endian; match Linux htole* for LE payloads.
#  ifndef htole16
#    define htole16(x) (static_cast<std::uint16_t>(x))
#  endif
#  ifndef htole32
#    define htole32(x) (static_cast<std::uint32_t>(x))
#  endif
#  ifndef htole64
#    define htole64(x) (static_cast<std::uint64_t>(x))
#  endif
#  ifndef le16toh
#    define le16toh(x) (static_cast<std::uint16_t>(x))
#  endif
#  ifndef le32toh
#    define le32toh(x) (static_cast<std::uint32_t>(x))
#  endif
#  ifndef le64toh
#    define le64toh(x) (static_cast<std::uint64_t>(x))
#  endif
#else
#  include <endian.h>
#endif
